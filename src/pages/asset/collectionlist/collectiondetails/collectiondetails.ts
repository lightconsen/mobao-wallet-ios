                ...
                if(res1.success == true){
                    ...
                    for(let key in list){
                        if(list[key].from == this.user.address){//发送  type=0
                            list[key].msg = this.getResult("assets.23")+list[key].to.slice(0,10)+'...'+list[key].to.slice(-10)+' ID '+list[key].amount+this.getResult("assets.43")+this.token.code;
                            list[key].type = 0;
                            list[key].code = this.token.code;
                        }else if(list[key].to == this.user.address){//接收 type=1
                            list[key].msg = this.getResult("assets.25")+list[key].from.slice(0,10)+'...'+list[key].from.slice(-10)+' ID '+list[key].amount+this.getResult("assets.43")+this.token.code;
                            list[key].type = 1;
                            list[key].code = this.token.code;
                        }
                    }
