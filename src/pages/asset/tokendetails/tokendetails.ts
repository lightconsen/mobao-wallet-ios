                        ...
                        if (res1.success == true) {
                            ...
                            for (let key in list1) {
                                ...
                                if (list1[key].from == this.user.address) {//发送  type=0
                                    list1[key].msg = this.getResult("assets.23") + list1[key].to.slice(0, 10) + '...' + list1[key].to.slice(-10) + this.getResult("assets.24");
                                    list1[key].type = 0;
                                    list1[key].code = this.token.code;
                                } else if (list1[key].to == this.user.address) {//接收 type=1
                                    list1[key].msg = this.getResult("assets.25") + list1[key].from.slice(0, 10) + '...' + list1[key].from.slice(-10) + this.getResult("assets.24");
                                    list1[key].type = 1;
                                    list1[key].code = this.token.code;
                                }
                            }
                            ...
                            if (res1.data.result.length == 0) {
                                ...
                                for (let key in list) {
                                    if (list[key].from == this.user.address) {//发送  type=0
                                        list[key].msg = this.getResult("assets.23") + list[key].to.slice(0, 10) + '...' + list[key].to.slice(-10) + this.getResult("assets.24");
                                        list[key].type = 0;
                                        list[key].code = this.token.code;
                                    } else if (list[key].to == this.user.address) {//接收 type=1
                                        list[key].msg = this.getResult("assets.25") + list[key].from.slice(0, 10) + '...' + list[key].from.slice(-10) + this.getResult("assets.24");
                                        list[key].type = 1;
                                        list[key].code = this.token.code;
                                    }
